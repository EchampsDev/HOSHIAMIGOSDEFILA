...
export class LocalAlbumRepository implements AlbumRepository {
  ...
  private async write(album: AlbumDocument) { window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...album, updatedAt: new Date().toISOString() })); window.dispatchEvent(new Event('brattypolitan-album-change')) }
  ...
  async savePage(page: ScrapbookPage) { const capacity = pageCapacity(page); if (capacity.used > 4 || capacity.stickersUsed > 10) throw new Error('Una cara admite cuatro recuerdos principales y diez stickers.'); const album = await this.read(); await this.write({ ...album, pages: album.pages.map((current) => current.id === page.id ? copy(page) : current) }) }
