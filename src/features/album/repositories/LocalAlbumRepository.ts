...
export class LocalAlbumRepository implements AlbumRepository {
  ...
  async savePage(page: ScrapbookPage) { if (pageCapacity(page).used > 4) throw new Error('Una cara no puede contener más de cuatro elementos.'); const album = await this.read(); await this.write({ ...album, pages: album.pages.map((current) => current.id === page.id ? copy(page) : current) }) }
