...
export function ScrapbookCoverArtwork({ bookmarkPage, onBookmark }: { bookmarkPage: number; onBookmark?: () => void }) {
  ...
  return <><span className="cover-page-edge" aria-hidden="true" /><SpiralBinding cover /><span className="cover-kicker">BRATTYPOLITAN</span><strong className="cover-title">Historias<br />Que inspiran</strong><span className="cover-hoshi-message"><i className="four-point-star cover-four-point-star is-active" aria-hidden="true"><i className="four-point-star__core" /></i><b>Hoshi sigue brillando</b></span><span className="cover-mark"><i className="cover-footer-mark" aria-hidden="true" /></span><span className={`cover-bookmark marker-${markerSide}`} role={onBookmark ? 'button' : undefined} aria-label={onBookmark ? `Ir a página ${bookmarkPage}` : undefined} onClick={(event) => { if (!onBookmark) return; event.stopPropagation(); onBookmark() }}><i>{bookmarkPage}</i></span><small className="cover-open-label">clic para abrir</small></>
}
