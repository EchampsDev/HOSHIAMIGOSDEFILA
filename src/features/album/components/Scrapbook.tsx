...
type Props = { state: BookState; page: ScrapbookPage | null; leftPage?: ScrapbookPage | null; rightPage?: ScrapbookPage | null; bookmarkPage: number; viewerId?: string | null; selectedId?: string | null; editable?: boolean; revealAll?: boolean; canInspectAll?: boolean; reactionsByElement?: Record<string, string[]>; navigationLocked?: boolean; viewMode: ReaderViewMode; paperTheme: ReaderPaperTheme; onBookmark: () => void; onPrevious: () => void; onNext: () => void; onSelect?: (pageId: string, id: string) => void; onLike?: (pageId: string, element: AlbumElement) => void; onElementPointerDown?: (event: React.PointerEvent<HTMLDivElement>, pageId: string, element: AlbumElement, mode: GestureMode) => void; direction: 'next' | 'previous' }
...
export function Scrapbook({ state, page, leftPage, rightPage, bookmarkPage, viewerId, selectedId, editable, revealAll, canInspectAll, reactionsByElement, navigationLocked = false, viewMode, paperTheme, onBookmark, onPrevious, onNext, onSelect, onLike, onElementPointerDown, direction }: Props) {
  const paper = (item: ScrapbookPage) => <><AlbumPaper page={item} viewerId={viewerId} selectedId={selectedId} editable={editable} revealAll={revealAll} canInspectAll={canInspectAll} reactionsByElement={reactionsByElement} showBinding={false} onSelect={(id) => onSelect?.(item.id, id)} onLike={(element) => onLike?.(item.id, element)} onElementPointerDown={(event, element, mode) => onElementPointerDown?.(event, item.id, element, mode)} />{bookmarkPage === item.pageNumber && <span className={`album-page-bookmark marker-${item.pageNumber % 2 === 0 ? 'left' : 'right'}`} aria-hidden="true"><i>{item.pageNumber}</i></span>}</>
