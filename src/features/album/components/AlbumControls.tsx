...
export function AlbumControls({ state, pageNumber, pageCount, bookmarkPage, presenting, paused, locked = false, viewMode, paperTheme, onViewMode, onPaperTheme, onBookmarkPage, onPrevious, onNext, onIndex, onPresent, onPause, onResume }: Props) {
  ...
  return <nav className={`album-controls${locked ? ' is-locked' : ''}`} aria-label="Controles de la libreta" aria-disabled={locked}>
    ...
    <fieldset className="album-reader-settings" disabled={locked}><legend>Vista</legend><button type="button" className={viewMode === 'BOOK' ? 'is-active' : ''} aria-pressed={viewMode === 'BOOK'} onClick={() => onViewMode('BOOK')}>Libreta completa</button><button type="button" className={viewMode === 'SINGLE' ? 'is-active' : ''} aria-pressed={viewMode === 'SINGLE'} onClick={() => onViewMode('SINGLE')}>Pantalla completa · una hoja</button></fieldset>
    <fieldset className="album-reader-settings" disabled={locked}><legend>Papel</legend><button type="button" className={paperTheme === 'CREAM' ? 'is-active' : ''} aria-pressed={paperTheme === 'CREAM'} onClick={() => onPaperTheme('CREAM')}>{viewMode === 'SINGLE' ? 'Blanco' : 'Crema'}</button><button type="button" className={paperTheme === 'BLACK' ? 'is-active' : ''} aria-pressed={paperTheme === 'BLACK'} onClick={() => onPaperTheme('BLACK')}>Negro</button></fieldset>
  </nav>
}
