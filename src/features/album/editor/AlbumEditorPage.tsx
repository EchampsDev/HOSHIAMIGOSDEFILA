...
type Gesture = { id: string; mode: 'move' | 'resize'; startX: number; startY: number; layout: ElementLayout }
...
function AlbumEditorWorkspace() {
  ...
  const pointerDown = (event: ReactPointerEvent<HTMLElement>, element: AlbumElement, mode: 'move' | 'resize') => { if (element.layout.locked) return; event.preventDefault(); gesture.current = { id: element.id, mode, startX: event.clientX, startY: event.clientY, layout: element.layout }; event.currentTarget.setPointerCapture(event.pointerId) }
