...
export function useAlbum() {
  ...
  useEffect(() => {
    ...
    void repository.getAlbum().then(setAlbum)
  }, [repository])
  const currentPage = album?.pages[pageNumber - 1] ?? null
  const savePage = useCallback((page: ScrapbookPage) => {
    setAlbum((current) => current ? { ...current, pages: current.pages.map((item) => item.id === page.id ? page : item) } : current)
    void repository.savePage(page).catch(() => setSyncError('No se pudo guardar. Inicia sesión con una cuenta administradora y revisa sus permisos.'))
  }, [repository])
