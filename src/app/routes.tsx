...
function PublicRoute({ children }: { children: ReactNode }) { const session = useGoogleSession(); if (session.isLoading) return <AccessLoading />; if (isConstellationContributor(session.role) && !session.isAdmin) return <Navigate to="/taller-constelacion" replace />; return <>{children}</> }
