...
export function NewsDetailPage() {
  ...
  return <Layout><article className="news-detail"><Link className="news-back-link" to="/#novedades">← Todas las novedades</Link><NewsCarousel images={item.images} fallbackAlt={item.carouselAlt || item.title} /><div className="news-detail-copy"><time dateTime={item.publishedAt}>{item.publishedAt ? new Date(item.publishedAt).toLocaleDateString('es-MX', { day: '2-digit', month: 'long', year: 'numeric' }) : ''}</time><h1>{item.title}</h1><p>{item.description}</p><NewsSocialLinks item={item} /></div></article></Layout>
}
