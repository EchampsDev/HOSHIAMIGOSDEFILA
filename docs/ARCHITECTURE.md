...
Las reacciones están separadas del documento de página mediante `src/features/reactions` y `elementReactions/{elementId_userId}`. La interfaz sólo ofrece la acción a personas autenticadas con Google; el repositorio y las reglas impiden reaccionar en nombre de otra cuenta. Cada reacción conserva además una instantánea del nombre público para mostrar quiénes dieron me gusta sin exponer sus identificadores.
