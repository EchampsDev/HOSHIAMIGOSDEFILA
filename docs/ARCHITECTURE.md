...
Las fotografías se cargan primero al bucket privado R2 y la aportación guarda exclusivamente su referencia. Mientras el registro está pendiente, el Worker permite lectura sólo al propietario o a administración. Después de aprobar, `approvedMedia/{id}` expone únicamente `objectKey` y fecha de aprobación; el Worker consulta esa proyección para servir el binario públicamente sin revelar autor, contenido ni datos de moderación.
